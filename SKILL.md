...
2. **Targeted base pull**: `shopify theme pull --theme <live_theme_id> --only 'templates/*' --only 'config/*' --only 'sections/*' --only 'snippets/*' --only 'blocks/*' --only 'layout/*' --only 'assets/*.css' --only 'assets/*.js' --path .theme-forge/base-cache/` (~10-15 sec). Gets fresh code, settings, and templates from the live theme.
