...
2. **Resolve roles for the current case.** For each role in `anchor_map.roles`:
   ...
   - **Cross-verify signal.** If `cross_verify: "failed"` on a resolved role, still run extraction but tag every resulting variance with `confidence: "rejected"` and `notes: "anchor cross-verify failed — live/dev elements likely unrelated; re-run /theme-forge intake-anchors <section> to re-resolve before trusting this variance"`. `rejected` means: emit so the user sees it, but refine-section MUST NOT attempt a fix (it would edit the wrong element). Listed in the decision report under "rejected variances" for user review; never promoted to in-scope without an intake re-run. This replaces the v0.21 `confidence: "low"` treatment, which let refine-section still act on the bad pairing.
   - **Source-binding parity signal (v0.23).** The anchor map may carry a per-role `source_binding: { live, dev, parity, citation }` block emitted by `intake-anchors` Step 6.5. Treat `parity` as follows:
     - `"both_confirmed"` — both sides' winners sit inside the role's Liquid-declared locus. Strong signal. Run extraction normally; do NOT downgrade confidence even if v0.22 cross-verify is borderline (source-binding over-rides the DOM heuristic).
     - `"partial"` — one side confirmed, the other inconclusive. Run extraction, but tag variances with `confidence: "low"` and `notes: "source-binding confirmed on <side> only; <other> inconclusive — verify role-bindings.json and dev snapshot"`. refine-section MAY proceed but should prefer conservative fixes.
     - `"mismatch"` — one side confirmed, the other side had its Stage 1 winner **rejected** by source-binding (winner outside the Liquid locus). Strongest wrong-pairing signal available. Tag every resulting variance with `confidence: "rejected"` and `notes: "source-binding parity mismatch — live/dev selectors likely point at different Liquid bindings. Edit intake-anchors/role-bindings.json or re-run with --ignore-source-binding <role>. Cited: <citation.filepath>:<citation.line>"`. refine-section MUST NOT act. Treated exactly like `cross_verify: "failed"` — surface for review, never auto-fix.
     - `"both_inconclusive"` — both sides inconclusive. Source-binding added no signal; fall back to the v0.22 `cross_verify` outcome above.
     - Absent `source_binding` block (pre-v0.23 anchor map) — treat as `both_inconclusive`; no new behavior.
