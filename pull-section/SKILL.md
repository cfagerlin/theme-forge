...
| Step 3.5 | Image verification result | append to transcript |
...
2. **Verify base-cache exists (HARD GATE).** Check that `.theme-forge/base-cache/config/settings_data.json` exists. This file is the source of truth for the base theme's image references, section settings, and content. Without it, pull-section cannot verify that image references are correctly migrated.
   - **If missing:** Run a targeted base pull:
     ```bash
     shopify theme pull --theme "$LIVE_THEME_ID" --store "$DEV_STORE" --path .theme-forge/base-cache --only config/settings_data.json --only "sections/*" --only "blocks/*" --only "templates/*" --only "snippets/*"
     ```
   - **If the pull fails:** STOP. Tell the user: "Cannot pull base theme data. Check store credentials and theme ID in `.theme-forge/config.json`."
   - **Do NOT proceed without base-cache.** The IMAGE SOURCING RULE and Image Verification Gate (Step 3.5) both depend on reading the base theme's actual settings values.
3. Resolve the page context (see "How the page is resolved" above)
4. Check for existing mapping at `.theme-forge/mappings/sections/{section-name}.json`
   - If missing, run `map-section` first to find the target section and assess compatibility
5. Load the mapping to determine the approach (JSON-only, JSON+CSS, extension section, custom section)
6. **Load global maps** (generated by scan):
...
7. **Load and apply learnings** from all files in `.theme-forge/learnings/` (see `references/learnings.md`)
...
8. **Find the target theme's CSS loading mechanism.** Search for how the theme includes stylesheets:
...
### Step 2.2: Build the Image Manifest

After completing the settings migration table, build a manifest of every image reference that must be migrated. This step consumes the Step 2.1 output — it does not re-analyze settings independently.

**For each setting classified as NATIVE, MAPPED, or EXTEND where the base schema type is `image` or `image_picker`:**

1. Look up the actual value in the base theme's `settings_data.json` (section-level settings) or the base template JSON (block-level settings)
2. Record the `shopify://shop_images/filename.ext` reference
3. If the value is empty or uses a schema default, skip it (no image to migrate)

**For each block instance in the base section:**

1. Read the block's settings from `settings_data.json` or template JSON
2. For every image-type setting on the block, record (block_type, block_id, setting_name, `shopify://shop_images/` value)
3. Note the block's position index — the target template must have a corresponding block with the same image

**Output the image manifest to the transcript:**

```
IMAGE MANIFEST: {section-name}
═══════════════════════════════════════════════════════════════
| Source                          | Setting        | Reference                              |
|---------------------------------|----------------|----------------------------------------|
| section.settings                | image          | shopify://shop_images/HP_Banner.png    |
| block[0] (slide)                | image          | shopify://shop_images/necklaces.png    |
| block[1] (slide)                | image          | shopify://shop_images/rings.jpg        |
| block[2] (slide)                | mobile_image   | shopify://shop_images/rings-mobile.png |
═══════════════════════════════════════════════════════════════
TOTAL: N images to verify after Step 3
```

**If the manifest is empty** (no image-type settings in base schema), note "No image settings — skipping image verification gate" and proceed. Step 3.5 is skipped.

**If base-cache is incomplete** (section not found in `settings_data.json`), check the base template JSON files in `.theme-forge/base-cache/templates/`. If the section's settings are in the template JSON rather than `settings_data.json`, read from there. Older Shopify themes (Liquid-based templates) store all section settings in `settings_data.json`, while newer themes (JSON templates) may split between template JSON and `settings_data.json`.
...
### Step 3.5: Image Verification Gate

**⛔ BLOCKING GATE: If the Image Manifest from Step 2.2 is non-empty, this gate MUST pass before proceeding to Step 4.**

After writing all JSON settings in Step 3, verify that every image in the manifest has a **matching** value in the target theme. This gate checks correctness, not just existence.

**For each manifest entry:**

1. Check the target theme's `config/settings_data.json` for the section-level image setting
2. Check the target template JSON (e.g., `templates/index.json`) for block-level image settings
3. Compare the target value against the manifest's recorded reference. PASS means the target value matches the manifest value exactly. A non-empty value that does NOT match the manifest is a MISMATCH (wrong image).

**Output the verification result:**

```
IMAGE VERIFICATION: {section-name}
═══════════════════════════════════════════════════════════════
| Source                   | Setting   | Status    | Expected                             | Target Value                       |
|--------------------------|-----------|-----------|--------------------------------------|------------------------------------|
| section.settings         | image     | ✓ PASS    | shopify://shop_images/HP_Banner.png  | shopify://shop_images/HP_Banner.png |
| block[0] (slide)         | image     | ✗ MISSING | shopify://shop_images/necklaces.png  | (empty)                            |
| block[1] (slide)         | image     | ✗ WRONG   | shopify://shop_images/rings.jpg      | shopify://shop_images/default.jpg  |
═══════════════════════════════════════════════════════════════
RESULT: 1/3 images verified — 1 MISSING, 1 WRONG
```

**If any entry is MISSING or WRONG:**
- Go back to the base theme's settings and find the correct `shopify://shop_images/` URL from the manifest
- Write the manifest's recorded value to the target theme's `settings_data.json` or template JSON
- Re-run the verification
- **Maximum 2 retry attempts.** If after 2 retries the gate still fails, report the specific failures to the user via `AskUserQuestion` and wait for direction. The image may not exist in the base theme's settings (e.g., stored in a metafield or deleted from the store).
- **Do NOT proceed to Step 4 with missing images.** Missing images mean broken screenshots, which means the visual comparison in Step 4 is comparing against an incomplete dev site. Every subsequent variance analysis will be wrong.

**If the manifest was empty** (noted in Step 2.2), skip this gate entirely.
